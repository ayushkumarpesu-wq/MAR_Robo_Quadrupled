...
def initialize_sim( dt, body_pos, fixed = False ):
    ...
    # add robot
    body_id = p.loadURDF("ayushbot.urdf", body_pos, useFixedBase=fixed)
    joint_ids = []
...
def step_simulation( body_id, body_pos, body_orn, body2feet ):
    #robot properties
...
if __name__ == '__main__':
    dT = 0.005
    bodyId, jointIds = initialize_sim( dt = dT, body_pos = [0,0,0.18], fixed = False )
    ...
    for k_ in range(0,N_steps):
        ...
        step_simulation( bodyId, pos, orn, bodytoFeet ) #simulates the robot to the target position
